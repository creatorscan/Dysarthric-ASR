...
train_set=all_speakers
train_dev=all_speakers
recog_set="all_speakers"
...
expdir=exp/${expname}_finetune
mkdir -p ${expdir}
#cp pretrained_models/asr/* $expdir/
#cp pretrained_models/lm/train*/* $lmexpdir/
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    ${cuda_cmd} --gpu ${ngpu} ${expdir}/train.log \
        ...
        --verbose ${verbose} \
        --resume ${resume} \
        --asr-init pretrained_models/asr/model.last10.avg.best \
        --train-json ${feat_tr_dir}/data_${bpemode}${nbpe}.json \
        --valid-json ${feat_tr_dir}/data_${bpemode}${nbpe}.json
fi

if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    echo "stage 5: Decoding"
    #if [ ! -s ${expdir}/results/model.last10.avg.best ]; then

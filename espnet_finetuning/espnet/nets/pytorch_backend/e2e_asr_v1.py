...
class Reporter(chainer.Chain):
    ...
    def report_cyc(self, tts_loss, cyc_loss):
        reporter.report({'tts_loss': tts_loss}, self)
        reporter.report({'cyc_loss': cyc_loss}, self)
...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def forward(self, xs_pad, ilens, ys_pad, tts_model=None, spembs=None, asrtts=False, ttsasr=False, rnnlm=None):
        ...
        if asrtts:
            ...
            self.loss_att, best_hyps = self.dec.generate_forward(hs_pad, torch.tensor(hlens), ys_pad, self.recog_args, rnnlm=rnnlm, oracle_length=self.oracle_length)
            #else:
            ...
            #self.loss_att = self.asr_to_tts(tts_model, fake_loss_att, best_hyps, ilens, xs_pad, spembs)
...
#        xlens_tts = to_device(self, torch.from_numpy(np.array([y.shape[0] for y in ys])).long())
#        xlens_tts = sorted(xlens_tts, reverse=True)
#        xs_tts = to_device(self, pad_list([y.long() for y in ys], 0))
...
#        return policy_loss
#
#    def asr_to_tts(self, tts_model, fake_loss, best_hyps, ilens, xs_pad, spembs):

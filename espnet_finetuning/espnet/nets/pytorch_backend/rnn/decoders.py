...
class Decoder(torch.nn.Module, ScorerInterface):
    ...
    def forward(self, hs_pad, hlens, ys_pad, strm_idx=0, tgt_lang_ids=None, ttsasr=None):
        """Decoder forward
        ...
        for i in six.moves.range(olength):
            if ttsasr:
                att_c = torch.zeros([hs_pad.size(0), hs_pad.size(-1)]).cuda()
                if i > 0:
    ...
    def generate_forward(self, hs_pad, hlens, ys_pad, recog_args, strm_idx=0, topk=0, maxlenratio=0.4, minlenratio=0.3, rnnlm=None, tgt_lang_ids=None, oracle_length=False):
        """Decoder forward
        ...
        ys_in_rep_pad = ys_in_rep.view(n_bb, ys_in_pad.size(1))
        ...
        # get dim, length info
        batch = hs_pad.size(0) # as we repeat here
        rbatch = ys_in_pad.size(0) # as we repeat here
        if oracle_length:
            olength = ys_out_pad.size(1)
        else:
            olength = maxlen
        ...
        for (i, y_hat), y_true in zip(enumerate(ys_hat),#.detach().cpu().numpy()),
            ...
            #    break
            if sampling == 'argmax':
                lsoft = F.log_softmax(y_hat, dim=-1)
                gen_list_probs = torch.topk(lsoft, beam, dim=-1)
                gen_list = gen_list_probs[1].view(beam, -1)
                gen_probs = gen_list_probs[0].view(beam, -1)
            elif sampling == 'multinomial':
                gen_list = []
                gen_probs = []
                lsoft = F.log_softmax(y_hat, dim=-1)
                soft = F.softmax(y_hat, dim=-1)
                for j in range(olength):
                    try:
                        #gen_ids = torch.multinomial(soft[j], num_samples=beam)
                        gen_ids = to_device(self, torch.from_numpy(np.random.choice(len(soft[j]), size=beam, p=soft[j].cpu().detach().numpy())))
                    except RuntimeError as e:
                        import ipdb; ipdb.set_trace()
                    gen_list.append(gen_ids)
                    gen_probs.append(torch.index_select(lsoft[j], -1, gen_ids))
                gen_list = torch.stack(gen_list).view(beam, -1)
                gen_probs = torch.stack(gen_probs).view(beam, -1)
                #idx_hat = np.array([torch.multinomial(F.softmax(torch.from_numpy(y_hat[j]), dim=-1), num_samples=4) for j in range(olength)])
            idx_hat = gen_list.detach().cpu().numpy()
            y_list.append(gen_list)
            p_list.append(gen_probs)
            if self.verbose > 0 and self.char_list is not None:
                idx_true = y_true[y_true != self.ignore_id]
                #seq_hat = [self.char_list[int(idx)] for idx in idx_hat]
                seq_true = [self.char_list[int(idx)] for idx in idx_true]
                ...
                for idx in idx_hat:
                    seq_hat = [self.char_list[int(ids)] for ids in idx]
                    seq_hat = "".join(seq_hat)
        ...
        #        y_str = "".join([self.char_list[int(idx)] for idx in y_seq])
        #        logging.info("generation[%d]: %.4f " % (i, sample_loss.data[i]) + y_str)
        self.loss = -to_device(self, torch.cat(p_list))
        #fake_ys_out_pad = to_device(self, pad_list(y_list, self.ignore_id).view(-1,olength))
        ...
        if rnnlm is not None:
            with torch.no_grad():
                sos = fake_ys_out_pad.data.new([self.sos] * len(y_list))
                rnnlm_state, lmz = rnnlm.predictor(None, sos)
                lm_loss = F.cross_entropy(lmz, fake_ys_out_pad[:, 0], reduction='none')
                for i in six.moves.range(1, len(y_list[0])):
                    rnnlm_state, lmz = rnnlm.predictor(rnnlm_state, fake_ys_out_pad[:, i - 1])
                    #if self.rnnloss == 'ce':
                    #logging.info("Using CE loss for RNNLoss")
                    loss_i = F.cross_entropy(lmz, fake_ys_out_pad[:, i], reduction='none')
                    ...
                    lm_loss += loss_i
                lm_loss = lm_loss / len(y_list[0])
            self.loss = self.loss.view(batch*beam, olength).mean(1)
        else:                                                                   
            self.loss = self.loss.view(batch*beam, olength).mean(1)
        return self.loss, lm_loss, y_list

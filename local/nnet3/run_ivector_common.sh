...
if [ $stage -le 1 ]; then
  ...
  steps/compute_cmvn_stats.sh ${data_all}_sp/{,log,data} || exit 1;
...
if [ $stage -le 2 ]; then
  ...
  steps/compute_cmvn_stats.sh ${data_all}_sp_hires${feat_affix}/{,log,data} || exit 1;
  ...
  steps/compute_cmvn_stats.sh ${data_all}_hires${feat_affix}/{,log,data} || exit 1;
...
if [ $stage -le 4 ]; then
  ...
  steps/online/nnet2/train_ivector_extractor.sh \
      --cmd "$train_cmd" --nj 10 --num-processes 1 \

...
if [ $stage -le 1 ]; then
    ...
    if [ ! -s $data_all/feats.scp ]; then
     local/torgo_make_mfcc.sh \
        --cmd "$train_cmd" --nj 15 --mfcc-config-dys $mfcc_config_dys \
        $data_all || exit 1;
    fi
    steps/compute_cmvn_stats.sh $data_all/{,log,data} || exit 1;
...
if [ $stage -le 2 ]; then
    ...
    for spk in $speakers; do
        echo $spk > $tmp/$spk
        utils/subset_data_tr_cv.sh --cv-spk-list "$tmp/$spk" $data_all \

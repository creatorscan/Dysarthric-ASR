...
# Copyright 2021  Brno University (Author: Murali Karthick baskar)
...
speakers="M02 M03 M04 M05 MC01 MC02 MC03 MC04"
...
# run those things.
    local/nnet3/run_ivector_common.sh --stage 5 \
...
if [ $stage -le 15 ]; then
    ...
    for spk in $speakers; do
        ...
        tdnn_opts="l2-regularize=0.03 dropout-proportion=0.0"
...
if [ $stage -le 16 ]; then
    for spk in $speakers; do
        (
        python -m steps/nnet3/chain/train.py \
    ...
    for spk in $speakers; do
        ...
        for data in $tests; do
            ...
            steps/nnet3/decode.sh \
                ...
                --nj $nspk --cmd "$train_cmd"  --num-threads 4 \

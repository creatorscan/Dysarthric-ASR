...
awk '{spk=$1; spk_clean=gensub("^.*-([^_]+_[^_]+_[^_]+).*","\\1","g",spk); print spk " " spk_clean }'  $srcdir/spk2utt   > $srcdir/spk2clean
awk '{MAP[$2]=MAP[$2] " " $1}; END{for(clean in MAP) print clean MAP[clean]}'                          $srcdir/spk2clean > $srcdir/clean2spk

#total, cv, train number of speakers
#N=$(cat $srcdir/spk2utt | wc -l)
N=$(cat $srcdir/clean2spk | wc -l)
...
awk '{print $1}' $srcdir/clean2spk | shuffle_list.pl --srand $seed > $trndir/_tmpf_randspk.clean

#split the train/cv
head -n $N_spk_cv $trndir/_tmpf_randspk.clean  > $cvdir/_tmpf_cvspk.clean
tail -n $N_spk_trn $trndir/_tmpf_randspk.clean > $trndir/_tmpf_trainspk.clean

# Expand spk back
awk '
k==0{SPK_clean[$1]=1}
k==1 && ($1 in SPK_clean) {for(i=2;i<=NF;i++) SPK_noise[$i]=1}
k==2 && ($1 in SPK_noise) {print}' k=0 $trndir/_tmpf_trainspk.clean k=1 $srcdir/clean2spk k=2 $srcdir/spk2utt > $trndir/_tmpf_trainspk

awk '
k==0{SPK_clean[$1]=1}
k==1 && ($1 in SPK_clean) {for(i=2;i<=NF;i++) SPK_noise[$i]=1}
k==2 && ($1 in SPK_noise) {print}' k=0 $cvdir/_tmpf_cvspk.clean k=1 $srcdir/clean2spk k=2 $srcdir/spk2utt | grep origx > $cvdir/_tmpf_cvspk
